...
def create_event_list():
    url = "https://www.naadyogacouncil.com/en/events/?tribe_paged=1&tribe_event_display=list&tribe-bar-date=2021-01-01"
    parsed = urlsplit(url)
    domain_name = parsed.netloc
    scheme = parsed.scheme
    resp = get_html(url)
    soup = BeautifulSoup(resp, "html.parser")
    events = soup.findAll("div", {'class': 'type-tribe_events'})
    print(len(events))
    links = []
    for event in events:
        # link = scheme + "://" + domain_name + event.select_one("a").attrs.get("href")
        link = event.select_one("a").attrs.get("href")
        links.append(link)
...
def get_details(data):
    ...
    card = {
        'title': data.get('name', " "),
        # 'location': data.get('location', dict()).get('name'),
        'location': data.get('location', dict()).get('name'),
        'organiser': data.get('performer'),
...
def classify_url(url) -> bool:
    parsed = urlsplit(url)
    if 'event-single' in parsed.path.split("/"):
        return True
    return False
